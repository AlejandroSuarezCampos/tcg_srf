...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfValido($_POST['csrf'] ?? null)) {
        header('Location: mazos?error=' . urlencode('La página ha caducado, inténtalo de nuevo.'));
        exit;
    }
    ...
    if ($accion === 'crear') {
        ...
        $destino = 'mazos?mazo=' . (int) $_POST['id_mazo'];
        ...
        $destino = 'mazos?mazo=' . $idMazo . ($res['ok'] ? '&ok=1' : '&error=' . urlencode($res['error']));
    }
...
// Una fila por JUGADOR, no por copia: todas las copias de un mismo jugador son
// intercambiables (mismas estadísticas) y solo una puede estar en la alineación
// a la vez, así que no tiene sentido listar 200 veces la misma carta común. El
// agrupado lo hace ya la consulta (ver `listarJugadoresDisponibles()`), que
// además trae `copias`.
//
// Se pide SIEMPRE, y no solo con un mazo abierto, porque la cifra de la
// cabecera también sale de aquí. Antes esa cifra hacía su propia consulta
// entera y contaba COPIAS, así que la cabecera decía «6.372 jugadores
// disponibles» mientras el selector de abajo decía «592» en la misma pantalla.
$porCromo = $db->listarJugadoresDisponibles($id_usuario);
...
<header class="cabecera">
  ...
  <div class="wrap cabecera-contenido">
    ...
    <div class="cabecera-datos">
      ...
      <div class="dato"><b><?= count($porCromo) ?></b><span>Jugadores disponibles</span></div>
    </div>
  </div>
</header>
          ...
          <ul class="lista-mazos">
            <?php foreach ($mazos as $m): ?>
              ...
              <li class="mazo-fila<?= $mazoActivo && $mazoActivo['id_mazo'] === $m['id_mazo'] ? ' es-activo' : '' ?>">
                <a href="mazos?mazo=<?= $m['id_mazo'] ?>" class="mazo-enlace">
          ...
          <aside class="mazo-taller-selector" aria-label="Jugadores disponibles">
            ...
            <?php if (empty($porCromo)): ?>
              <div class="vacio">
                ...
                <a class="btn btn-primary" href="sobres">Ir a sobres</a>
              </div>
            <?php else: ?>
              ...
              <ul class="carta-lista mazo-selector-lista" id="m-lista">
                <?php foreach ($porCromo as $c): ?>
                  <?php
                  $idCromo  = (int) $c['id_cromo'];
                  $cantidad = (int) $c['copias'];
