...
> Documento de traspaso, versión 7.9 (2026-08-23).
...
>
> **Lo grande de la v7.9: el NODO DE BLOQUEO**, el STOP del mapa de cadena
> (migración `045`): corta el paso hasta cumplir requisitos —rango S en lo
> anterior, X goles en un partido, porterías a cero y ocho tipos más— y se
> abre solo. Léete el changelog de la v7.9 antes de tocar cadenas.
>
> **De la v7.8, que fue auditoría de entorno:** al 2026-08-23 esta copia **ya
> no tiene `.git`** (lo que decía la v7.7 sobre remoto y rama `master` dejó de
> valer). Lo de las 16 migraciones sin aplicar **ya está resuelto** — era la
> base local desfasada, no un bug; se importó el volcado de producción y las
> suites quedaron en verde. Ve al bloque "⚠️⚠️ ESTADO EXACTO AL 2026-08-23"
> (justo antes de "Cómo arrancar en un chat nuevo") antes de asumir nada del
> resto del documento sobre git.
>
...
> ---
>
> ## v7.8 — Auditoría de entorno (2026-08-23): sin `.git` y 16 migraciones sin aplicar
>
> Esta versión no añade funcionalidad — es una **comprobación del estado real**
> del proyecto contra lo que decía la v7.7, porque las dos cosas habían
> divergido bastante entre el 2026-08-21 y hoy. Detalle completo en el aviso
> "ESTADO EXACTO AL 2026-08-23" justo debajo de este changelog; aquí solo el
> resumen de qué se encontró y por qué importa.
>
> 1. **El `.git` con remoto que describía la v7.7 ya no está en `tcg_srf`.**
>    `git status` da "not a git repository". El único repo git relacionado que
>    queda accesible es uno obsoleto en `C:\xampp\htdocs\imgs\tcg_srf` (5
>    commits, código de una versión muy anterior — `consultas.php` de 36 KB
>    frente a las ~490 KB actuales), que **no hay que confundir con el
>    proyecto**. Hay además un `.rar` de 49 MB creado hoy en
>    `C:\xampp\htdocs\` que parece un backup manual reciente, sin verificar su
>    contenido. No se sabe qué pasó con el `.git` de la v7.7 — puede que
>    Alejandro haya vuelto a trabajar desde una copia descomprimida; no se ha
>    investigado más porque no toca código.
> 2. **Aparecieron 16 migraciones que el documento no mencionaba**,
>    `029_dificultad_por_nodo.sql` a `044_nodos_libres_start_y_requisitos.sql`,
>    escritas y con comentarios propios, pero **ninguna aplicada** ni en `tcg`
>    ni en `tcg_prueba`. Entre ellas: equipos editables con escudo/universo,
>    tutorial de bienvenida, universo movido de equipo a carta, rareza
>    "Numerada" con cupo, sobre inicial + duelos amistoso sin apuesta, apuesta
>    con varias cartas, cadenas con visibilidad restringida, calibración de
>    dificultad PvE por preset simulado, dificultad por nodo individual, rango
>    S por margen de victoria (sustituye a goles+cero encajados),
>    recalibración del malus de coherencia, y editor de nodos en posición
>    libre con requisitos nuevos de cadena.
> 3. **El código ya no es compatible con la base de datos tal cual está.**
>    `db/consultas.php` ya llama a columnas (`universo`, `cupo_numerado`,
>    `tutorial_paso`...) y tablas (`duelo_apuesta_cartas`,
>    `cadena_nodo_dificultad`...) que esas 16 migraciones crean y que hoy no
>    existen. Al correr `db/pruebas/correr_todas.php`, 4 suites nuevas fallan
>    por eso exactamente (`probar_dificultad_nodo`, `probar_apuesta_cartas`,
>    `probar_calibracion_pve`, `probar_equipos_y_tutorial`); las suites viejas
>    siguen en verde. No es el bug de corrupción de Aria del §8 — es
>    simplemente que faltan migraciones por aplicar, probablemente vía
>    `db/actualizar_produccion.sql` (54,8 KB, ya existe en el repo como dump
>    consolidado).
> 4. **Ficheros nuevos no reflejados en el §2** (arquitectura): páginas
>    `cadena.php`, `cadenas.php`, `diagnostico.php`, `info.php`;
>    `panel/cadena_editor.php`, `panel/cadenas.php`, `panel/equipos.php`,
>    `panel/mantenimiento.php`; `partials/assets.php`,
>    `partials/ceremonia_cofre.php`, `partials/ficha_carta.php`,
>    `partials/presentacion_duelo.php`, `partials/tutorial.php`;
>    `assets/ajax/cadena_admin.php`, `assets/ajax/equipos.php`,
>    `assets/ajax/mantenimiento.php`, `assets/ajax/tutorial.php`,
>    `assets/ajax/duelo_penalti.php`; `db/herramientas/` (mantenimiento,
>    reparar_compos, convertir_a_webp); `db/actualizar_produccion.sql`; una
>    carpeta `img/` en la raíz (separada de `assets/img/`) con
>    `Afinidades/, Cromos/, Escudos/, Sobres/, iconos/, marcos/, perfil/,
>    plantillas/`.
> 5. **Nada de esto es una regresión de código** — `php -l` sigue sin errores
>    en todo el proyecto, y el `.htaccess`/`404.php` que la v7.7 daba como
>    "pendiente de commitear" siguen exactamente igual, ya integrados sin
>    sorpresas.
>
> **No se ha tocado ni una línea de código ni de migración en esta sesión.**
> El §1 (tabla de estado), el §2 (arquitectura de ficheros) y las secciones
> que describirían equipos/tutorial/rareza numerada/cadenas con dificultad por
> nodo siguen sin actualizar en el resto del documento — esta versión
> documenta **que existen y dónde mirar** (los `.sql` de `029` a `044` traen
> comentarios; el código que los usa está localizado arriba), no los explica
> sección a sección. Es el trabajo que queda para la próxima sesión si
> Alejandro quiere esto documentado a fondo, o para cuando se decida aplicar
> las migraciones y verificar en caliente qué hace cada sistema.

---

## v7.9 — EL NODO DE BLOQUEO: el STOP del mapa de cadena (migración `045`)

Lo pidió Alejandro junto con la Cadena del Abuelo: *"un nodo de stop, que no
puedas pasar hasta cumplir requisitos (pasarte todo en rango S, meter tantos
goles en un partido, y muchas más)"*.

**Es un tipo de nodo nuevo, no un candado colgado del partido siguiente.** La
diferencia importa: `mapaCadena()` ya abre un nodo cuando ALGUNO de sus
predecesores está superado, así que un bloqueo puesto en medio corta el paso
**sin ninguna regla nueva** — lo único que cambia es de dónde sale su
`superado`, que en vez de ganar un partido es cumplir sus requisitos. Y como
se dibuja en el mapa, se ve venir desde lejos, que es lo que hace un STOP.
Un candado en el partido siguiente habría obligado a leerlo en cada nodo y a
decidir qué se enseña cuando de una bifurcación salen varios.

**Los once tipos de requisito** (`Tcg::REQUISITOS_NODO`). Cinco son nuevos y de
nodo; los seis restantes son los mismos de la `044` a nivel de cadena y se
evalúan **con el mismo código**, no con una copia:

| Tipo | Qué exige |
|---|---|
| `rango_previos` | Todos los partidos de un camino hasta aquí en rango ≥ X |
| `nodos_cadena` | Ganar X partidos **distintos** de la cadena |
| `victorias` | Ganar X partidos (repetir el mismo cuenta) |
| `goles_partido` | Meter X goles en **un mismo** partido |
| `porteria_cero` | Ganar X partidos sin encajar |
| `cadena` · `cromo` · `nivel_album` · `monedas` · `duelos` · `rareza` | Los seis de la `044` |

Cuatro cosas que hay que saber antes de tocarlo:

1. **`rango_previos` es "EXISTE UN CAMINO", no "todos los ancestros"**, mismo
   criterio y misma razón que `caminoPerfectoHastaCofre()` (§15.12): la cadena
   se recorre ELIGIENDO camino, así que exigir todos los ancestros obligaría a
   jugarse las dos ramas de cada bifurcación. Con un mapa recto las dos
   lecturas dan lo mismo — solo se distinguen con una bifurcación, y por eso
   la prueba monta una a propósito.
2. **La dificultad de un requisito no es un adorno.** Las cinco están siempre
   abiertas y `mejor_rango` se guarda por dificultad, así que un
   `rango_previos` sin dificultad fijada **se puede granjear entero en Fácil**
   y abrir un STOP pensado para Extremo. Es el mismo agujero que ya avisaba el
   §15.12. Por eso `dificultad` es columna propia y no un segundo número
   metido en `valor` (la lección que dejó escrita la `044`).
3. **Un bloqueo sin requisitos DEJA PASAR, y un requisito sin partidos delante
   también.** Las dos son deliberadas y por el mismo motivo: es el lado seguro
   por el que fallar. Si cerrasen, un STOP recién puesto o mal cableado
   dejaría la cadena muerta para siempre y sin forma de saber por qué. La
   contrapartida es que `rango_previos` colgado donde no hay partidos detrás
   **parece configurado y no exige nada**.
4. ⚠️ **Un STOP solo bloquea si es paso OBLIGADO.** Como basta con que
   cualquier predecesor esté superado, una arista paralela que lo rodee lo
   deja de adorno. Salió al probarlo en el navegador: el nodo siguiente seguía
   abierto porque conservaba su arista antigua. Al montar la cadena, hay que
   quitar el atajo.

**`cadena_progreso` empieza a guardar goles.** Hasta ahora el marcador se
calculaba entero, se enseñaba y se tiraba, así que "mete 5 goles" no se podía
ni comprobar. Nuevas `mas_goles` (RÉCORD, con `GREATEST` — un partido flojo
después no te quita la marca, mismo criterio que `mejor_rango`) y
`porterias_cero` (cuenta acumulada, y **solo si además se ganó**: un 0-0
perdido en los penaltis no es la hazaña que premia el requisito).

### Y un fallo de la `044` que estaba ahí desde entonces

El `<select>` de tipo de nodo del editor solo tenía `partido` y `cofre`.
`abrirModalNodo()` hace `fn_tipo.value = n.tipo`, y **asignar a un `<select>`
un valor que no tiene ninguna `<option>` no falla**: deja el select como
estaba. Así que al abrir una **casilla de salida** el desplegable decía
"Partido", y darle a *Guardar nodo* **la convertía de verdad en un partido**,
borrando el nodo especial sin avisar ni dejar rastro. Ya están los cuatro
tipos en el desplegable.

**Prueba nueva: `db/pruebas/probar_nodo_bloqueo.php`** (21 comprobaciones), ya
dentro de `correr_todas.php` — **6 suites en verde**. Verificado además en el
navegador de punta a punta sobre la Cadena del Abuelo: el control corta el
paso, lista lo que falta con su barra, y al llegar a S en Extremo en los
cuatro partidos del camino se abre solo y desbloquea el siguiente. Los datos
de esa prueba se limpiaron: la base local volvió a ser copia fiel de
producción.

### ⚠️ `ionos/` ES LA CARPETA DE DESPLIEGUE, Y ESTABA CONGELADA EN LA v7.7

Lo confirmó Alejandro: **de `ionos/` es de donde se sube a tcgfrontier.es** (por
FTP, su contenido, no la carpeta). Y llevaba sin refrescarse desde el
**2026-08-21**: le faltaban ~3.000 líneas de `db/consultas.php` y **todo el
código de las migraciones `029` a `044`** — equipos, tutorial, universo en la
carta, rareza Numerada, sobre inicial, apuesta con varias cartas, dificultad
por nodo, calibración PvE. Ninguna de esas funciones existía en la web aunque
**la base de datos de producción sí tenía ya el esquema nuevo**.

Ya está sincronizada (2026-08-23). Lo que hay que saber:

1. **`ionos/` es hoy idéntica a la raíz salvo UNA línea**: el
   `charset=utf8mb4` del DSN en `ionos/db/consultas.php`. Comprobado fichero a
   fichero: todo lo demás coincide byte a byte. El propio fichero lleva el
   aviso en esa línea.
2. **Cuatro cosas NO se copian nunca**: `branding/` (documentación interna;
   nunca estuvo, aunque el LEEME decía que sí), `.claude/`, `db/conexion.php`
   (credenciales — el servidor ya tiene la suya) y sobre todo
   **`db/conexion.local.php`**, que es la config de XAMPP y que
   `db/conexion.php` PREFIERE si existe: subirla dejaría la web conectando a
   una base local inexistente y el sitio entero caído.
3. **El `.htaccess` de la raíz manda, no el de `ionos/`.** El LEEME decía que
   la copia usaba ruta relativa en `ErrorDocument`, pero **Apache no admite
   ruta relativa ahí** (la manda como texto literal al navegador). La raíz ya
   trae la versión correcta, con el arreglo del *"Forbidden"* de la portada de
   tcgfrontier.es. Ese punto del LEEME está tachado y explicado.
4. **`db/tcg.sql` estaba en el esquema de la `028`** y el LEEME mandaba
   importarlo: hoy eso montaría una instalación rota. Revolcado con todo hasta
   la `045`, en la raíz y en `ionos/`. ⚠️ **Y solo vale para instalar de cero**
   — importarlo sobre la web en marcha se llevaría por delante usuarios,
   colecciones y progreso. Para actualizar, migraciones sueltas: el LEEME tiene
   ahora una sección propia («Actualizar un despliegue QUE YA ESTÁ EN MARCHA»).
5. **Orden al desplegar: primero la migración, después los ficheros.** Las
   migraciones son aditivas, así que el código viejo aguanta el esquema nuevo;
   al revés no. Es literalmente el fallo que costó la sesión de la v7.8.
6. Copia de seguridad del `ionos/` anterior en
   `C:\xampp\htdocs\_ionos_backup_20260823\` (615 ficheros), fuera del proyecto
   para que no se despliegue ni la escanee nadie.

**Para poner la web al día hace falta**: aplicar `045_nodo_bloqueo.sql` en la
base de IONOS (las `029`–`044` ya estaban) y subir el contenido de `ionos/`.

**Lo que NO se ha tocado**: las 75+ formaciones siguen pendientes y necesitan
cambiar el modelo antes (hoy una formación es `[nDF, nMC, nDC]` sumando 10 y
las coordenadas se DERIVAN, así que **no caben 75**: hay 36 combinaciones
matemáticas y ~10 con sentido futbolístico). El plan acordado es que cada
formación declare sus 11 huecos con rol y posición explícita (`["DF", 14,
73]`); como el rol sigue siendo POR/DF/MC/DC, `PESOS_LINEA` y el motor de
combate no se tocan y las 8 actuales se siguen derivando de `lineas`.
Alejandro eligió que las 75+ sean **las de Inazuma Eleven, con sus nombres**.
...
> ## ⚠️⚠️ ESTADO EXACTO AL 2026-08-23 — LEE ESTO ANTES QUE EL BLOQUE DEL 21 ⚠️⚠️
>
> Auditoría de esta sesión (no un cambio de código, solo comprobación del
> entorno real contra lo que decía este documento). Dos hallazgos grandes:
>
> **1. YA NO HAY `.git` EN `tcg_srf`.** Todo lo que el bloque del 2026-08-21 y
> el §0 dicen sobre "hay `.git` y hay remoto en `master`" **es falso para esta
> copia ahora mismo** — `git status` en `C:\xampp\htdocs\tcg_srf` da `fatal:
> not a git repository`. Lo que sí existe es un repo git **obsoleto y sin
> relación con el código actual** en `C:\xampp\htdocs\imgs\tcg_srf` (5 commits,
> el último "0.3 Versión estable para pruebas", `consultas.php` de 36 KB frente
> a las ~490 KB de hoy — **no lo confundas con el proyecto**), y un
> `C:\xampp\htdocs\tcg_srf-master.rar` de 49 MB creado el 2026-08-23 a las
> 13:25 que parece un backup manual reciente (contenido sin verificar). Trata
> el proyecto como **sin control de versiones** hasta que Alejandro diga lo
> contrario; no asumas commits, ramas ni que "está a la par con origin".
>
> **2. ~~HAY 16 MIGRACIONES SIN APLICAR~~ — RESUELTO EL MISMO 2026-08-23.**
> Se encontró la base local 16 migraciones por detrás (`029`–`044`): faltaban
> `universo`, `cupo_numerado`, `duelo_apuesta_cartas`,
> `cadena_nodo_dificultad`, `tutorial_paso`, `pos_x`/`pos_y`… mientras
> `db/consultas.php` ya las llamaba, y **4 suites fallaban por eso**
> (`probar_dificultad_nodo`, `probar_apuesta_cartas`,
> `probar_calibracion_pve`, `probar_equipos_y_tutorial`). **No era ningún bug
> de código ni la corrupción de Aria del §8: era la base local desfasada.**
>
> Se arregló **importando el volcado de producción** (IONOS `dbs16038604`,
> descargado por Alejandro) sobre la `tcg` local, previa copia de seguridad en
> `C:\Users\assas\Downloads\backup_tcg_local_20260823.sql`. Tras ello: 44
> tablas, las columnas nuevas presentes, `Montaña` sigue en `c3b1` (§5.3), y
> **las suites en verde**. Producción ya tenía las 16 aplicadas; el problema
> era solo local.
>
> ⚠️ **La lección, que ya iba por la tercera vez** (la `013` en la v6, la `016`
> en la v7, y ahora dieciséis de golpe): **comprueba el esquema local cada
> sesión.** Y ojo al síntoma nuevo — cuando el código va por delante de la
> base, no falla una migración en silencio: fallan las PRUEBAS con errores de
> columna inexistente, que es fácil confundir con una regresión de código.
> `db/conexion.php` mira si existe `db/conexion.local.php` y solo entonces usa
> la base local; en producción no existe y va contra IONOS.
>
> Lo que SÍ sigue como decía el bloque del 21: los usuarios `Claude` (id 9) y
> `CPU` (id 10), el `.htaccess` con el `ErrorDocument 404 /404.php` (nota: es
> `/404.php`, no `/tcg_srf/404.php` — el propio `.htaccess` explica que en
> producción el proyecto es la raíz) y `404.php` en la raíz, ambos ya
> integrados sin más pendiente que ver. `php -l` sobre todo el proyecto sigue
> sin errores.
>
> **Resumen de qué hay construido desde la v7.7 y no documentado en ningún
> sitio de este fichero**, a falta de que alguien escriba las secciones
> propiamente (ver también el changelog v7.8 más abajo): equipos editables con
> escudo/descripción/universo (`panel/equipos.php`), tutorial de bienvenida
> paso a paso (`partials/tutorial.php`), universo (SRF/IE) movido de equipo a
> carta, rareza "Numerada" con cupo y numeración, sobre inicial gratuito +
> duelos "amistoso" sin apuesta, apuestas con varias cartas por lado, cadenas
> con visibilidad restringida a usuarios elegidos, calibración de dificultad
> PvE por preset con simulación, dificultad configurable por nodo individual,
> rango S recalibrado por margen de victoria en vez de goles+cero encajados,
> recalibración del malus de coherencia, y editor de cadenas con nodos en
> posición libre (píxeles) y 4 tipos nuevos de requisito. Páginas nuevas:
> `cadena.php`, `cadenas.php`, `panel/cadena_editor.php`, `panel/cadenas.php`,
> `panel/equipos.php`, `panel/mantenimiento.php`. Antes de tocar cualquiera de
> estos sistemas, lee el `.sql` de su migración (están en
> `db/migraciones/029_*.sql` a `044_*.sql`, cada uno con comentarios) y el
> código PHP que lo usa — este documento todavía no lo cuenta.
...
3. Verificar que el repo está intacto:
   ...
   ```
4. **Comprueba si hay `.git` — al 2026-08-23 esta copia NO lo tiene:**
   ```
   git status -sb
   ```
   Si da `fatal: not a git repository`, no asumas commits ni ramas: trabaja
   como si no hubiera control de versiones y considera `git init` antes de
   cambios grandes, avisando primero. No confundas el proyecto con el repo
   obsoleto de `C:\xampp\htdocs\imgs\tcg_srf` (5 commits, código de una versión
   muy antigua) ni con el `.rar` de backup que pueda haber en `htdocs` — ni uno
   ni otro es el control de versiones del proyecto actual.
   *(Entre el 2026-08-11 y el 2026-08-21 sí hubo `.git` con remoto en
   `github.com/AlejandroSuarezCampos/tcg_srf`, rama `master` — ver v7.7 y el
   aviso "ESTADO EXACTO AL 2026-08-23" más arriba. Si tu copia sí tiene `.git`
   apuntando ahí, entonces esa parte del historial sigue vigente; compruébalo,
...
5. **Comprobar que la BD está al día. Una sola consulta lo dice todo:**
   ...
   ✅ **Resuelto el 2026-08-23**: la base local llegó a estar 16 migraciones
   por detrás (`029`–`044`) mientras `db/consultas.php` ya las llamaba, y 4
   suites fallaban por eso. Se arregló importando el volcado de producción; al
   2026-08-23 local está al día hasta la `045` (el nodo de bloqueo, v7.9) y
   las 6 suites pasan. Si alguna vez vuelves a ver fallos de columna/tabla
   inexistente en las pruebas, **es esto otra vez, no un bug de código**:
   compara `SHOW TABLES` / `SHOW COLUMNS` contra el último `db/migraciones/*`
   antes de sospechar del motor.
...
8. **Comprueba si `ionos/` está al día con la raíz** — es la carpeta de
   despliegue y desde el 2026-08-23 es regla fija refrescarla al cerrar
   cualquier sesión que toque código (ver el aviso justo antes de "Cómo
   arrancar en un chat nuevo" y la § v7.9):
   ```
   diff -r --brief C:\xampp\htdocs\tcg_srf\db C:\xampp\htdocs\tcg_srf\ionos\db
   ```
   Salida esperada, siempre estas cuatro líneas y ninguna más:
   ```
   Only in db: conexion.local.php
   Only in db: conexion.php
   Only in ionos/db: conexion.php.example
   Files db/consultas.php and ionos/db/consultas.php differ
   ```
   Las tres primeras son correctas siempre (las credenciales nunca están en
   los dos sitios a la vez, `.example` es solo de la copia de despliegue) y no
   dependen de ninguna sesión. La cuarta es la línea del `charset=utf8mb4`, la
   única adaptación real de código. **Si aparece cualquier OTRA línea** —un
   fichero de más, uno de menos, o `consultas.php` con más de esa diferencia—
   `ionos/` se quedó atrás en una sesión anterior: refréscala antes de seguir,
   no acumules la diferencia.
...
- **Control de versiones:** ⚠️ **al 2026-08-23 esta copia NO tiene `.git`** —
  ver el aviso "ESTADO EXACTO AL 2026-08-23" al principio del documento antes
  de asumir nada de lo que sigue en este punto. Entre el 2026-08-11 y el
  2026-08-21 sí hubo `.git` con remoto en
  `github.com/AlejandroSuarezCampos/tcg_srf`, rama `master`, empujada y a la
  par con `origin` (la rama de trabajo fue `minijuegos-x75` hasta el merge del
  2026-08-13, `e4b0426`; desde entonces todo iba directo a `master` — v7.7).
  Comprueba `git status -sb` en tu copia: si hay `.git`, ese historial sigue
  siendo válido; si no, trátalo como proyecto sin control de versiones y no
  confundas con el repo obsoleto de `C:\xampp\htdocs\imgs\tcg_srf` (5 commits,
  código de una versión antigua, no relacionado con el estado actual).
  El `.git` que hubo se creó como red de seguridad antes de una reescritura
  destructiva del sistema de cajas/sobres (§14), y el primer commit
  (`"Checkpoint antes de reescribir..."`) era el estado previo a ella — útil si
  algo hay que revertir y ese historial reaparece. **Nunca `--force`, nunca
  reescribir historial** sin que lo pida explícitamente Alejandro. Commitea y
  sube **solo cuando él lo pida**.
...
> ## ⚠️ REGLA FIJA DESDE EL 2026-08-23: `ionos/` SE REFRESCA AL CERRAR TODA SESIÓN QUE TOQUE CÓDIGO
>
> Se decidió tras encontrar `ionos/` congelada en la v7.7 mientras la raíz
> llevaba dos días avanzando por delante (v7.8) — le faltaban ~3.000 líneas y
> el código entero de las migraciones `029` a `044`. **La raíz sigue siendo la
> única copia de trabajo**: se edita y se prueba ahí, contra `tcg` local, como
> siempre. Lo que cambia es que **ya no basta con refrescar `ionos/` "cuando
> Alejandro lo pida"** — es un paso obligatorio de cierre de sesión, igual que
> el resumen de qué cambió y por qué.
>
> **Si una sesión tocó cualquier fichero de código, plantilla, CSS o JS de la
> raíz** (no si solo tocó `branding/`, datos de prueba, o si solo se
> investigó sin editar nada), antes de darse por terminada:
>
> 1. Refrescar `ionos/` desde la raíz:
>    ```
...
>    ```
> 2. Volver a poner el `charset=utf8mb4` en `ionos/db/consultas.php` (el propio
>    fichero lleva el aviso en esa línea — se pierde en cada refresco porque la
>    raíz no lo lleva). **Es la única adaptación manual que queda.**
> 3. Si hubo migración nueva, comprobar que su `.sql` esté en
>    `ionos/db/migraciones/` (el robocopy ya la copia; esto es solo para
>    confirmarlo) y decir en el resumen de cierre qué migración falta aplicar
>    en la base de IONOS — **nunca aplicarla tú a la base remota**, eso lo
>    hace Alejandro a mano en phpMyAdmin cuando decide desplegar.
> 4. Verificar que la única diferencia entre raíz e `ionos/` sigue siendo esa
>    línea del charset:
>    ```
>    diff -r --brief C:\xampp\htdocs\tcg_srf\db C:\xampp\htdocs\tcg_srf\ionos\db
>    ```
>    Si sale cualquier otra cosa, algo del robocopy falló o alguien tocó
>    `ionos/` a mano — investigar antes de cerrar.
>
> **`ionos/` sigue sin servirse ni probarse en local.** Es una copia de
> ficheros para subir por FTP, no un segundo entorno de desarrollo: no tiene
> `conexion.local.php` ni falta que le hace. Todo el desarrollo y las pruebas
> (`php -l`, `db/pruebas/correr_todas.php`, el navegador) se siguen haciendo
> contra la raíz, exactamente igual que siempre.
>
> Detalle completo de qué es `ionos/`, qué NO viaja nunca (`conexion.local.php`
> sobre todo — dejaría el sitio real conectando a una base local inexistente)
> y cómo actualizar un despliegue ya en marcha: § v7.9 más abajo y
> `ionos/LEEME-DESPLIEGUE-IONOS.md`.
