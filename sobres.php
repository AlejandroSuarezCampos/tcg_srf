...
/* El sobre de bienvenida pendiente (migración `039`). Sale en su propio bloque
   arriba del todo, fuera de las cajas de expansión: es gratis, se abre una vez
   y el tutorial lo pide. Enterrado entre los de pago no lo encontraba nadie. */
...
<main id="contenido" class="seccion wrap">
  ...
  <?php if ($sobreInicial): ?>
  <section class="sobre-inicial" aria-labelledby="sobreInicialTitulo">
    <div class="sobre-inicial-texto">
      <span class="pastilla pastilla-titular">Gratis · una sola vez</span>
      <h2 id="sobreInicialTitulo"><?= htmlspecialchars($sobreInicial['nombre']) ?></h2>
      <p class="t-body-sm t-dim">
        Trae <b><?= (int) $sobreInicial['cantidad'] ?> jugadores</b> con las posiciones
        justas para montar tu primer once: un portero, cuatro defensas, cuatro medios
        y dos delanteros. No es al azar — es el equipo con el que vas a empezar.
      </p>
    </div>
    <?php /* Botón y no formulario: se abre por AJAX igual que el resto, para
             que el sobre de bienvenida tenga su ceremonia. Es el primer sobre
             que ve alguien — precisamente el que no puede abrirse con una
             recarga de página y once cartas apareciendo de golpe en una lista. */ ?>
    <button type="button" class="btn btn-primary" id="btnSobreInicial"
            data-id-sobre="<?= (int) $sobreInicial['id_sobre'] ?>"
            data-nombre="<?= htmlspecialchars($sobreInicial['nombre']) ?>"
            data-imagen="<?= htmlspecialchars($sobreInicial['imagen']) ?>">
      <i class="ph ph-package" aria-hidden="true"></i> Abrir el sobre de bienvenida
    </button>
  </section>
  <?php endif; ?>
