-- ############################################################################
--
--   ACTUALIZACIÓN ACUMULADA  ·  migraciones 046 a 048
--   Superliga Frontier TCG
--
--   Un solo fichero con TODO lo que hay que pasar por la base desde la 046.
--   Es el equivalente exacto a ejecutar, en este orden:
--
--       046_mundial_posiciones_y_jamaica.sql
--       047_afinidad_webp.sql
--       048_reparar_es_final.sql
--
--   Las tres siguen existiendo por separado en `db/migraciones/`: este fichero
--   NO las sustituye, las junta. Si ya has pasado alguna suelta, puedes
--   ejecutar este igualmente — las tres son idempotentes y volver a pasarlas
--   deja exactamente el mismo resultado.
--
--   ────────────────────────────────────────────────────────────────────────
--   QUÉ HACE, EN UNA LÍNEA CADA UNA
--
--   046  Las siete selecciones del mundial (Australia, China, Congo, España,
--        Jamaica, Japón y Países Bajos) reciben la posición de su alineación
--        oficial, su afinidad, rareza y estadísticas por escalón, y las rutas
--        de imagen correctas. Jamaica recupera su plantilla.
--
--   047  `afinidad.imagen` apuntaba a `.png` y los iconos son `.webp`: las
--        cuatro rutas daban 404 y el hexágono de afinidad no se veía en NINGUNA
--        parte del sitio.
--
--   048  Deshace los «nodos finales» que nadie marcó, del bug de
--        `actualizar_nodo`. Deja para revisar a mano qué cadenas se quedan sin
--        cofre final.
--
--   ⚠️ LO QUE NO ESTÁ AQUÍ, Y NO FALTA. Las 45 formaciones nuevas NO llevan
--      migración: el catálogo vive en `Tcg::FORMACIONES` (PHP), y lo que la
--      base guarda es solo la CLAVE ("442", "4231"…) en `mazos.formacion`,
--      `formaciones_usuario.formacion` y las formaciones congeladas del duelo.
--      Esas columnas son `varchar(8)` y la clave más larga mide 5, así que
--      entran sin tocar nada. Subir el código basta.
--
--   ────────────────────────────────────────────────────────────────────────
--   CÓMO PASARLO
--
--       mysql -u USUARIO -p BASE < db/migraciones/ACTUALIZAR_046_A_048.sql
--
--   o pegándolo entero en phpMyAdmin. Al final salen tres consultas de
--   comprobación; míralas antes de dar el despliegue por bueno.
--
-- ############################################################################
