...
class Tcg
{
	...
	/**
	 * Copias ofrecidas en un intercambio que sigue vivo (migración `053`).
	 *
	 * La cuarta vía por la que una carta cambia de dueño, y por tanto la cuarta
	 * que tiene que retenerla. Mismo criterio y mismo motivo que la de arriba:
	 * mientras la oferta esté abierta esa copia todavía puede irse a otro, así
	 * que no se puede vender, descartar, alinear ni apostar.
	 *
	 * ⚠️ EL PLAZO SE COMPRUEBA AQUÍ DENTRO (`vence > NOW()`), no solo en la
	 *    limpieza periódica. Si la retención dependiera de que alguien haya
	 *    pasado a marcar la oferta como caducada, unas cartas seguirían
	 *    congeladas hasta la siguiente visita a la pantalla de intercambios —y
	 *    con 22 jugadores eso puede ser días. Vencida es vencida: la copia
	 *    queda libre en el mismo instante, y `caducarIntercambios()` solo pone
	 *    al día el estado para que la oferta deje de pintarse.
	 */
	private const SQL_COPIAS_EN_INTERCAMBIO = "
		SELECT ida.id_coleccion
		FROM intercambio_da ida
		INNER JOIN intercambios i ON i.id_intercambio = ida.id_intercambio
		WHERE i.estado = 'abierto' AND i.vence > NOW()
	";
	...
	const TUTORIAL_PASOS = [
		[
			"clave" => "bienvenida", "pagina" => "landing", "destino" => "landing",
		...
		[
			"clave" => "navegacion", "pagina" => "landing", "destino" => "landing",
		...
		[
			"clave" => "sobres", "pagina" => "sobres", "destino" => "sobres",
		...
		[
			"clave" => "coleccion", "pagina" => "plantilla", "destino" => "plantilla",
		...
		[
			"clave" => "bloqueo", "pagina" => "plantilla", "destino" => "plantilla",
		...
		[
			"clave" => "album", "pagina" => "plantilla", "destino" => "plantilla?ver=todas",
		...
		[
			"clave" => "mazo_crear", "pagina" => "mazos", "destino" => "mazos",
		...
		[
			"clave" => "mazo_formacion", "pagina" => "mazos", "destino" => "mazos",
		...
		[
			"clave" => "mazo_jugadores", "pagina" => "mazos", "destino" => "mazos",
		...
		[
			"clave" => "mazo_equilibrio", "pagina" => "mazos", "destino" => "mazos",
		...
		[
			"clave" => "mazo", "pagina" => "mazos", "destino" => "mazos",
		...
		[
			"clave" => "cadenas", "pagina" => "cadenas", "destino" => "cadenas",
		...
		[
			"clave" => "partido", "pagina" => "landing", "destino" => "landing",
		...
		[
			"clave" => "duelos", "pagina" => "duelos", "destino" => "duelos",
		...
		[
			"clave" => "mercado", "pagina" => "mercado", "destino" => "mercado",
		...
		[
			"clave" => "misiones", "pagina" => "misiones", "destino" => "misiones",
		...
		[
			...
			"clave" => "ajustes", "pagina" => "perfil", "destino" => "perfil#panel-ajustes",
		...
		[
			"clave" => "fin", "pagina" => "landing", "destino" => "landing",
	...
	/**
	 * Qué cromos tiene, como mapa `id_cromo => true` para preguntar con
	 * `isset()`.
	 *
	 * Es lo que `plantilla.php` necesita para apagar en el modo «todas» las
	 * fichas que le faltan al jugador: un sí/no por cromo. Lo sacaba de
	 * `listarColeccionUsuario()`, que trae la fila ENTERA de cada COPIA —cinco
	 * JOIN, nombre, imagen, estadísticas— y se quedaba solo con el id: con una
	 * colección completa eran 7.104 filas y 26 MB de PHP para construir un
	 * array de 592 booleanos.
	 */
	public function cromosPoseidos($id_usuario) {
		$stmt = $this->pdo->prepare("SELECT DISTINCT id_cromo FROM coleccion WHERE id_usuario = :id");
		$stmt->execute([":id" => $id_usuario]);
	...
	/**
	 * Lo que este usuario puede poner en venta: una fila por CROMO, con
	 * `copias` diciendo cuántas tiene libres.
	 *
	 * Agrupa por el mismo motivo que `listarJugadoresDisponibles()`: dos copias
	 * del mismo jugador son intercambiables —el propio `mercado.php` ya decía
	 * que pintar una ficha por copia «colgaba el navegador al abrir el modal»—,
	 * así que se agrupaban en PHP después de traerlas todas. Ahora agrupa la
	 * base de datos y no viajan las que sobran: con una colección completa eran
	 * 7.104 filas para pintar 592.
	 *
	 * `id_coleccion` es el de UNA copia libre cualquiera; `publicarAnuncio()`
	 * vuelve a comprobar que sea suya y que no esté bloqueada.
	 *
	 * (El `LEFT JOIN cadena_numeracion` que había aquí no se usaba en ningún
	 * sitio: no aportaba ni una columna al SELECT.)
	 */
	public function listarColeccionVendible($id_usuario) {
		$sql = "
			SELECT MIN(col.id_coleccion) AS id_coleccion, COUNT(*) AS copias,
			...
			WHERE col.id_usuario = :id_usuario
				...
				AND col.id_coleccion NOT IN (" . self::SQL_COPIAS_EN_INTERCAMBIO . ")
			GROUP BY c.id_cromo, c.nombre, c.imagen,
				c.ataque, c.defensa, c.tecnica,
				eq.nombre, c.universo, r.id_rareza, r.nombre,
				af.nombre, af.imagen
	...
	public function publicarAnuncio($id_coleccion, $id_usuario, $precio) {
		...
		// Y con una carta ofrecida en un intercambio vivo, por lo mismo: está
		// prometida a alguien. (migración `053`)
		$stmtInt = $this->pdo->prepare(
			"SELECT 1 FROM (" . self::SQL_COPIAS_EN_INTERCAMBIO . ") ofrecidas
			 WHERE ofrecidas.id_coleccion = :id_coleccion LIMIT 1");
		$stmtInt->execute([":id_coleccion" => $id_coleccion]);
		if ($stmtInt->fetchColumn()) {
			return false;
		}
	...
	/* ======================================================================
	   INTERCAMBIOS  (migración `053`)
	   ----------------------------------------------------------------------
	   Cartas por cartas, sin monedas de por medio: para pagar ya está el
	   mercado de aquí arriba. Dos vías sobre la misma tabla, igual que una
	   sala de duelo y un reto son los dos `duelos`:

	     · OFERTA DIRIGIDA   `id_destinatario` = alguien. Se propone desde su
	                         perfil: "te doy estas, dame esas".
	     · ANUNCIO DEL TABLÓN `id_destinatario` = NULL. Lo ve todo el mundo, y
	                         admite las dos formas de cerrarse: aceptarlo tal
	                         cual (si pides cromos concretos) o contraofertar
	                         otra cosa (`id_origen`).

	   SIN TOPE DE VALOR, a propósito: una común por una SRF es un intercambio
	   válido. El mercado sí acota precios porque ahí hay monedas y una
	   horquilla que defender; aquí lo que se pacta es entre dos personas que
	   se conocen, y la liga son veintidós.
	   ====================================================================== */

	/** Cuántas cartas puede poner cada lado. En BD para poder tocarlo sin desplegar. */
	private function intercambioMaxPorLado() {
		return max(1, (int) $this->config("intercambio_cartas_max", 10));
	}

	/** Horas que vive una oferta antes de caducar y soltar las cartas. */
	private function intercambioHorasVida() {
		return max(1, (int) $this->config("intercambio_horas", 48));
	}

	/**
	 * Marca como caducadas las ofertas cuyo plazo ha pasado.
	 *
	 * NO es lo que libera las cartas —de eso se encarga el `vence > NOW()` de
	 * `SQL_COPIAS_EN_INTERCAMBIO`, que no espera a que nadie ejecute esto—,
	 * sino lo que hace que dejen de pintarse como si siguieran vivas. Se llama
	 * al abrir la pantalla de intercambios, que es el patrón que ya usa
	 * `cancelarSalasAbandonadas()` con los duelos: sin cron, y quien enciende
	 * la luz paga la limpieza.
	 */
	public function caducarIntercambios() {
		$this->pdo->exec("
			UPDATE intercambios
			   SET estado = 'caducado', resuelto = NOW()
			 WHERE estado = 'abierto' AND vence <= NOW()
		");
	}

	/**
	 * Copias libres de un cromo concreto que son de este usuario.
	 *
	 * El mismo criterio de "libre" que `listarColeccionVendible()`, escrito
	 * otra vez porque aquí hace falta bloquear las filas (`FOR UPDATE`) y
	 * devolver copias sueltas, no cromos agrupados. Es lo que convierte un
	 * "busco este cromo" en cartas de verdad en el momento de aceptar.
	 */
	private function copiasLibresDeCromo($id_usuario, $id_cromo, $cuantas) {
		$stmt = $this->pdo->prepare("
			SELECT col.id_coleccion
			  FROM coleccion col
			 WHERE col.id_usuario = :u
			   AND col.id_cromo = :c
			   AND col.bloqueada = 0
			 ...
			 FOR UPDATE
		");
		$stmt->execute([":u" => (int) $id_usuario, ":c" => (int) $id_cromo]);
		return array_map("intval", $stmt->fetchAll(PDO::FETCH_COLUMN));
	}

	/**
	 * Crea una oferta. `$da` y `$busca` son los dos [id_cromo => cantidad].
	 *
	 * LOS DOS LADOS SE PIDEN EN CROMOS, no en copias, y por el mismo motivo:
	 * dos copias del mismo cromo son intercambiables. El selector ya agrupa
	 * («×4» bajo la carta) porque pintar una ficha por copia colgaba el
	 * navegador —hay cuentas con 315.000—, así que no tiene de dónde sacar
	 * cuatro `id_coleccion` distintos para ofrecer cuatro copias. Aquí se
	 * eligen: `copiasLibresDeCromo()` reserva las que hagan falta dentro de la
	 * transacción y son esas las que quedan comprometidas.
	 *
	 * `$id_destinatario` NULL publica en el tablón. `$id_origen` marca que
	 * esto es una contraoferta a un anuncio.
	 */
	public function crearIntercambio($id_emisor, $id_destinatario, array $da, array $busca, $id_origen = null) {
		$id_emisor = (int) $id_emisor;
		$max = $this->intercambioMaxPorLado();

		$positivas = function ($n) { return $n > 0; };
		$da    = array_filter(array_map("intval", $da), $positivas);
		$busca = array_filter(array_map("intval", $busca), $positivas);

		if (!$da) {
			return ["ok" => false, "error" => "Tienes que poner al menos una carta.", "id" => null];
		}
		if (array_sum($da) > $max) {
			return ["ok" => false, "error" => "No puedes poner más de $max cartas por lado.", "id" => null];
		}
		if (array_sum($busca) > $max) {
			return ["ok" => false, "error" => "No puedes pedir más de $max cartas por lado.", "id" => null];
		}
		if ($id_destinatario !== null && (int) $id_destinatario === $id_emisor) {
			return ["ok" => false, "error" => "No puedes intercambiar contigo mismo.", "id" => null];
		}
		/* Un anuncio del tablón que ni pide cromos ni admite nada es un cartel
		   en blanco. Las dirigidas sí pueden ir sin `busca`: son un regalo, y
		   regalar cartas es legítimo. */
		if ($id_destinatario === null && !$busca) {
			return ["ok" => false, "error" => "Di qué cartas buscas a cambio.", "id" => null];
		}

		try {
			$this->pdo->beginTransaction();

			/* Se reservan copias de verdad para cada cromo ofrecido. Si de
			   alguno no hay suficientes libres —en un mazo, a la venta,
			   apostadas, protegidas o ya ofrecidas en otra oferta— no se crea
			   nada: media oferta es peor que ninguna. */
			$aOfrecer = [];
			foreach ($da as $idCromo => $cantidad) {
				$copias = $this->copiasLibresDeCromo($id_emisor, $idCromo, $cantidad);
				if (count($copias) < $cantidad) {
					$this->pdo->rollBack();
					return ["ok" => false, "id" => null,
					        "error" => "No tienes libres todas esas cartas. Comprueba que no estén "
					                 . "en un mazo, protegidas, a la venta, apostadas o ya ofrecidas."];
				}
				$aOfrecer = array_merge($aOfrecer, $copias);
			}

			// Contraoferta: el anuncio tiene que seguir vivo y no ser tuyo.
			$destino = $id_destinatario === null ? null : (int) $id_destinatario;
			if ($id_origen !== null) {
				$stmt = $this->pdo->prepare("
					SELECT id_emisor, estado, vence FROM intercambios
					 WHERE id_intercambio = :id AND id_destinatario IS NULL FOR UPDATE
				");
				$stmt->execute([":id" => (int) $id_origen]);
				$anuncio = $stmt->fetch(PDO::FETCH_ASSOC);
				if (!$anuncio || $anuncio["estado"] !== "abierto" || strtotime($anuncio["vence"]) <= time()) {
					$this->pdo->rollBack();
					return ["ok" => false, "error" => "Ese anuncio ya no está disponible.", "id" => null];
				}
				if ((int) $anuncio["id_emisor"] === $id_emisor) {
					$this->pdo->rollBack();
					return ["ok" => false, "error" => "Ese anuncio es tuyo.", "id" => null];
				}
				// La contraoferta va dirigida a quien publicó, se diga o no.
				$destino = (int) $anuncio["id_emisor"];
			}

			$this->pdo->prepare("
				INSERT INTO intercambios (id_emisor, id_destinatario, id_origen, vence)
				VALUES (:e, :d, :o, DATE_ADD(NOW(), INTERVAL :h HOUR))
			")->execute([
				":e" => $id_emisor,
				":d" => $destino,
				":o" => $id_origen === null ? null : (int) $id_origen,
				":h" => $this->intercambioHorasVida(),
			]);
			$id = (int) $this->pdo->lastInsertId();

			$insDa = $this->pdo->prepare(
				"INSERT INTO intercambio_da (id_intercambio, id_coleccion) VALUES (:i, :c)");
			foreach ($aOfrecer as $idCol) {
				$insDa->execute([":i" => $id, ":c" => $idCol]);
			}

			$insBusca = $this->pdo->prepare(
				"INSERT INTO intercambio_busca (id_intercambio, id_cromo, cantidad) VALUES (:i, :c, :n)");
			foreach ($busca as $idCromo => $cantidad) {
				$insBusca->execute([":i" => $id, ":c" => (int) $idCromo, ":n" => (int) $cantidad]);
			}
			...
			if ($this->pdo->inTransaction()) { $this->pdo->rollBack(); }
			return ["ok" => false, "error" => "No se ha podido crear el intercambio.", "id" => null];
		}
	}

	/**
	 * Cierra un intercambio: las cartas cambian de dueño en los dos sentidos.
	 *
	 * Todo dentro de una transacción y revalidando: entre que se pintó la
	 * pantalla y se pulsó el botón, las cartas que se piden pueden haberse
	 * vendido, metido en un mazo o apostado. Si falta una sola, no se mueve
	 * nada — media transferencia sería peor que ninguna.
	 */
	public function aceptarIntercambio($id_intercambio, $id_usuario) {
		$id_intercambio = (int) $id_intercambio;
		...
		try {
			...
			$stmt = $this->pdo->prepare("
				SELECT id_emisor, id_destinatario, id_origen, estado, vence
				  FROM intercambios WHERE id_intercambio = :id FOR UPDATE
			");
			$stmt->execute([":id" => $id_intercambio]);
			$inter = $stmt->fetch(PDO::FETCH_ASSOC);

			if (!$inter || $inter["estado"] !== "abierto" || strtotime($inter["vence"]) <= time()) {
				$this->pdo->rollBack();
				return ["ok" => false, "error" => "Esa oferta ya no está disponible."];
			}
			$emisor = (int) $inter["id_emisor"];
			if ($emisor === $id_usuario) {
				$this->pdo->rollBack();
				return ["ok" => false, "error" => "No puedes aceptar tu propia oferta."];
			}
			if ($inter["id_destinatario"] !== null && (int) $inter["id_destinatario"] !== $id_usuario) {
				$this->pdo->rollBack();
				return ["ok" => false, "error" => "Esa oferta no va dirigida a ti."];
			}

			/* ⚠️ CAE TODO LO QUE COLGABA DEL MISMO ANUNCIO, Y CAE AQUÍ, ANTES DE
			   BUSCAR LAS CARTAS — no al final, con el resto del cierre.

			   Quien acepta una contraoferta es quien publicó el anuncio, y su
			   anuncio tiene retenidas unas copias suyas. Si la contraoferta le
			   pide justo esas —que es el caso NORMAL: se contraoferta por lo
			   que se ha visto publicado—, buscarlas antes de soltar el anuncio
			   las encuentra ocupadas por él mismo y el trato se rechaza con un
			   "no tienes libres todas las cartas" incomprensible.

			   Soltarlo primero es correcto además de práctico: el anuncio se
			   cierra igualmente en este mismo commit, y si algo falla después,
			   la transacción lo devuelve todo a su sitio.

			   Lo mismo vale en el otro sentido: si lo aceptado ES el anuncio,
			   esto tumba sus contraofertas, que ya no tienen objeto. */
			$raiz = $inter["id_origen"] !== null ? (int) $inter["id_origen"] : $id_intercambio;
			$this->pdo->prepare("
				UPDATE intercambios SET estado = 'rechazado', resuelto = NOW()
				 WHERE estado = 'abierto' AND id_intercambio <> :yo
				   AND (id_intercambio = :raiz OR id_origen = :raiz2)
			")->execute([":yo" => $id_intercambio, ":raiz" => $raiz, ":raiz2" => $raiz]);

			// --- lo que entrega el emisor: tiene que seguir siendo suyo ------
			$stmt = $this->pdo->prepare("
				SELECT ida.id_coleccion
				  FROM intercambio_da ida
				  INNER JOIN coleccion col ON col.id_coleccion = ida.id_coleccion
				 WHERE ida.id_intercambio = :id AND col.id_usuario = :e
				 FOR UPDATE
			");
			$stmt->execute([":id" => $id_intercambio, ":e" => $emisor]);
			$delEmisor = array_map("intval", $stmt->fetchAll(PDO::FETCH_COLUMN));

			$stmtN = $this->pdo->prepare(
				"SELECT COUNT(*) FROM intercambio_da WHERE id_intercambio = :id");
			$stmtN->execute([":id" => $id_intercambio]);
			if (count($delEmisor) !== (int) $stmtN->fetchColumn()) {
				$this->pdo->rollBack();
				return ["ok" => false, "error" => "Quien te lo ofreció ya no tiene alguna de esas cartas."];
			}

			// --- lo que pide: se resuelve a copias libres de quien acepta ----
			$stmt = $this->pdo->prepare(
				"SELECT id_cromo, cantidad FROM intercambio_busca WHERE id_intercambio = :id");
			$stmt->execute([":id" => $id_intercambio]);

			$delAceptante = [];
			foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $fila) {
				$copias = $this->copiasLibresDeCromo($id_usuario, $fila["id_cromo"], (int) $fila["cantidad"]);
				if (count($copias) < (int) $fila["cantidad"]) {
					$this->pdo->rollBack();
					return ["ok" => false,
					        "error" => "No tienes libres todas las cartas que piden. "
					                 . "Comprueba que no estén en un mazo, protegidas, a la venta o apostadas."];
				}
				$delAceptante = array_merge($delAceptante, $copias);
			}

			// --- transferencia, en los dos sentidos --------------------------
			$mover = $this->pdo->prepare("
				UPDATE coleccion SET id_usuario = :u, bloqueada = 0, obtenida = NOW()
				 WHERE id_coleccion = :c
			");
			foreach ($delEmisor as $idCol) {
				$mover->execute([":u" => $id_usuario, ":c" => $idCol]);
			}
			foreach ($delAceptante as $idCol) {
				$mover->execute([":u" => $emisor, ":c" => $idCol]);
			}

			$this->pdo->prepare("
				UPDATE intercambios
				   SET estado = 'aceptado', id_cerro = :u, resuelto = NOW()
				 WHERE id_intercambio = :id
			")->execute([":u" => $id_usuario, ":id" => $id_intercambio]);

			/* Cualquier oferta viva que ofreciera una copia que acaba de
			   cambiar de dueño. No debería quedar ninguna —las copias estaban
			   retenidas—, salvo las de una oferta vencida que todavía nadie
			   ha marcado como caducada. */
			$todas = array_merge($delEmisor, $delAceptante);
			// Hoy no puede estar vacío —crear una oferta exige poner algo—, pero
			// sin esto un `IN ()` sería un error de sintaxis dentro de la
			// transacción que mueve las cartas. Una línea a cambio de eso.
			if ($todas) {
				$marcas = implode(",", array_fill(0, count($todas), "?"));
				$this->pdo->prepare("
					UPDATE intercambios SET estado = 'rechazado', resuelto = NOW()
					 WHERE estado = 'abierto' AND id_intercambio <> ?
					   AND id_intercambio IN (
					       SELECT id_intercambio FROM intercambio_da WHERE id_coleccion IN ($marcas)
					   )
				")->execute(array_merge([$id_intercambio], $todas));
			}

			$this->pdo->commit();
			return ["ok" => true, "error" => null,
			        "recibidas" => count($delEmisor), "entregadas" => count($delAceptante)];

		} catch (Exception $e) {
			if ($this->pdo->inTransaction()) { $this->pdo->rollBack(); }
			return ["ok" => false, "error" => "No se ha podido completar el intercambio."];
		}
	}

	/**
	 * Rechaza (si eres el destinatario) o retira (si eres el emisor) una
	 * oferta. Un solo método porque es el mismo UPDATE con el mismo efecto
	 * —soltar las cartas—; solo cambia quién puede y cómo se llama.
	 *
	 * Un anuncio del tablón no tiene destinatario, así que solo su emisor
	 * puede cerrarlo. Al retirarlo caen también sus contraofertas.
	 */
	public function cerrarIntercambio($id_intercambio, $id_usuario) {
		$id_intercambio = (int) $id_intercambio;
		$id_usuario     = (int) $id_usuario;

		$stmt = $this->pdo->prepare("
			SELECT id_emisor, id_destinatario FROM intercambios
			 WHERE id_intercambio = :id AND estado = 'abierto'
		");
		$stmt->execute([":id" => $id_intercambio]);
		$inter = $stmt->fetch(PDO::FETCH_ASSOC);
		if (!$inter) {
			return ["ok" => false, "error" => "Esa oferta ya no está abierta."];
		}

		$esEmisor = (int) $inter["id_emisor"] === $id_usuario;
		$esDestinatario = $inter["id_destinatario"] !== null
			&& (int) $inter["id_destinatario"] === $id_usuario;

		if (!$esEmisor && !$esDestinatario) {
			return ["ok" => false, "error" => "Esa oferta no es tuya."];
		}

		$this->pdo->prepare("
			UPDATE intercambios SET estado = :e, resuelto = NOW()
			 WHERE id_intercambio = :id AND estado = 'abierto'
		")->execute([":e" => $esEmisor ? "retirado" : "rechazado", ":id" => $id_intercambio]);

		if ($esEmisor) {
			$this->pdo->prepare("
				UPDATE intercambios SET estado = 'rechazado', resuelto = NOW()
				 WHERE estado = 'abierto' AND id_origen = :id
			")->execute([":id" => $id_intercambio]);
		}
	...
	/**
	 * Rellena cada oferta con sus cartas: `da` (copias, con arte) y `busca`
	 * (cromos, con arte y cantidad).
	 *
	 * En DOS consultas para toda la lista y no dos por oferta. Con veintidós
	 * jugadores la diferencia es teórica, pero el patrón de traerse las cartas
	 * de una en una es justo el que ya reventó el modal del mercado.
	 */
	private function hidratarIntercambios(array $ofertas) {
		if (!$ofertas) { return []; }

		$ids = array_map(function ($o) { return (int) $o["id_intercambio"]; }, $ofertas);
		$marcas = implode(",", array_fill(0, count($ids), "?"));

		$campos = "c.id_cromo, c.nombre, c.imagen, c.posicion, c.ataque, c.defensa, c.tecnica,
		           eq.nombre AS equipo, c.universo, r.id_rareza, r.nombre AS rareza,
		           af.nombre AS afinidad, af.imagen AS afinidad_imagen";
		$joins = "INNER JOIN equipos eq ON eq.id_equipo = c.id_equipo
		          INNER JOIN rarezas r ON r.id_rareza = c.id_rareza
		          INNER JOIN afinidad af ON af.id = c.id_afinidad";

		$stmt = $this->pdo->prepare("
			SELECT ida.id_intercambio, ida.id_coleccion, $campos
			  FROM intercambio_da ida
			  INNER JOIN coleccion col ON col.id_coleccion = ida.id_coleccion
			  INNER JOIN cromos c ON c.id_cromo = col.id_cromo
			  $joins
			 WHERE ida.id_intercambio IN ($marcas)
			 ORDER BY r.id_rareza DESC, c.nombre
		");
		$stmt->execute($ids);
		$da = [];
		foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $f) {
			$da[(int) $f["id_intercambio"]][] = $f;
		}

		$stmt = $this->pdo->prepare("
			SELECT ib.id_intercambio, ib.cantidad, $campos
			  FROM intercambio_busca ib
			  INNER JOIN cromos c ON c.id_cromo = ib.id_cromo
			  $joins
			 WHERE ib.id_intercambio IN ($marcas)
			 ORDER BY r.id_rareza DESC, c.nombre
		");
		$stmt->execute($ids);
		$busca = [];
		foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $f) {
			$busca[(int) $f["id_intercambio"]][] = $f;
		}

		foreach ($ofertas as &$o) {
			$id = (int) $o["id_intercambio"];
			$o["da"]    = $da[$id]    ?? [];
			$o["busca"] = $busca[$id] ?? [];
		}
		unset($o);
	...
	/** Las columnas comunes a los tres listados, con los dos nombres resueltos. */
	private const SQL_INTERCAMBIO_BASE = "
		SELECT i.id_intercambio, i.id_emisor, i.id_destinatario, i.id_origen,
		       i.estado, i.creado, i.vence,
		       ue.nombre AS emisor, ud.nombre AS destinatario,
		       (SELECT COUNT(*) FROM intercambios h
		         WHERE h.id_origen = i.id_intercambio AND h.estado = 'abierto') AS contraofertas
		  FROM intercambios i
		  INNER JOIN usuarios ue ON ue.id_usuario = i.id_emisor
		  LEFT JOIN usuarios ud ON ud.id_usuario = i.id_destinatario
	";

	/** Ofertas abiertas en las que este usuario es parte, dirigidas o suyas. */
	public function listarIntercambiosDe($id_usuario) {
		$stmt = $this->pdo->prepare(self::SQL_INTERCAMBIO_BASE . "
			 WHERE i.estado = 'abierto' AND i.vence > NOW()
			   AND (i.id_emisor = :u OR i.id_destinatario = :u2)
			 ORDER BY i.creado DESC
		");
		$stmt->execute([":u" => (int) $id_usuario, ":u2" => (int) $id_usuario]);
		return $this->hidratarIntercambios($stmt->fetchAll(PDO::FETCH_ASSOC));
	}

	/** El tablón: anuncios abiertos a cualquiera. Los propios también salen. */
	public function listarTablonIntercambios($limite = null) {
		$sql = self::SQL_INTERCAMBIO_BASE . "
			 WHERE i.estado = 'abierto' AND i.vence > NOW() AND i.id_destinatario IS NULL
			 ORDER BY i.creado DESC";
	...
	/** Una oferta suelta, con sus cartas. Null si no existe. */
	public function detalleIntercambio($id_intercambio) {
		$stmt = $this->pdo->prepare(self::SQL_INTERCAMBIO_BASE . " WHERE i.id_intercambio = :id");
		$stmt->execute([":id" => (int) $id_intercambio]);
		$fila = $stmt->fetch(PDO::FETCH_ASSOC);
		if (!$fila) { return null; }
		$hidratadas = $this->hidratarIntercambios([$fila]);
		return $hidratadas[0];
	}

	/**
	 * Cuántas ofertas esperan respuesta de este usuario. Es la cifra de la
	 * insignia: solo cuenta lo que le toca contestar A ÉL, no lo que ha
	 * enviado y sigue esperando.
	 */
	public function contarIntercambiosPendientes($id_usuario) {
		$stmt = $this->pdo->prepare("
			SELECT COUNT(*) FROM intercambios
			 WHERE estado = 'abierto' AND vence > NOW() AND id_destinatario = :u
		");
		$stmt->execute([":u" => (int) $id_usuario]);
	...
	/**
	 * Los JUGADORES que este usuario puede alinear: una fila por cromo, no una
	 * por copia, con `copias` diciendo cuántas tiene.
	 *
	 * Antes devolvía una fila por COPIA y `mazos.php` las agrupaba después en
	 * PHP, porque en el once da igual qué copia concreta se use: todas valen lo
	 * mismo y un cromo solo puede aparecer una vez. Eso significaba traer de la
	 * base de datos —con sus cinco JOIN y su subconsulta de rasgo— tantas filas
	 * como copias tuviera la persona, para tirar la mayoría acto seguido. Con
	 * una colección completa (592 cromos, 12 copias de cada) eran 6.372 filas y
	 * 12 MB de PHP para pintar 592; y las copias no tienen techo, así que era el
	 * único punto de esta pantalla que crecía sin parar.
	 *
	 * `id_coleccion` es el de UNA copia cualquiera (la más antigua). Sirve para
	 * mandarla en el formulario; `guardarCartasMazo()` vuelve a comprobar que
	 * sea suya y que pueda jugar, así que cuál sea da igual.
	 *
	 * El GROUP BY enumera todas las columnas y no solo `c.id_cromo` —del que
	 * todas dependen— porque MariaDB 10.4 no deduce esa dependencia: con
	 * `ONLY_FULL_GROUP_BY` activo (puede estarlo en el hosting y no en local)
	 * la consulta corta abortaría con «isn't in GROUP BY».
	 */
	public function listarJugadoresDisponibles($id_usuario) {
		$marcadores = implode(",", array_fill(0, count(self::POSICIONES_JUGABLES), "?"));
		$stmt = $this->pdo->prepare("
			SELECT
				MIN(col.id_coleccion) AS id_coleccion,
				COUNT(*) AS copias,
				c.id_cromo, c.nombre, c.posicion, c.imagen,
				c.ataque, c.defensa, c.tecnica,
				eq.nombre AS equipo, c.universo,
			...
			WHERE col.id_usuario = ?
				...
				AND col.id_coleccion NOT IN (SELECT id_coleccion FROM mercado WHERE activa = 1)
				/* Igual que una carta puesta a la venta: si está ofrecida en un
				   intercambio abierto todavía puede irse, y alinearla dejaría el
				   once cojo en cuanto el otro acepte. (migración `053`) */
				AND col.id_coleccion NOT IN (" . self::SQL_COPIAS_EN_INTERCAMBIO . ")
			GROUP BY c.id_cromo, c.nombre, c.posicion, c.imagen,
				c.ataque, c.defensa, c.tecnica,
				eq.nombre, c.universo, r.id_rareza, r.nombre,
				af.nombre, af.imagen
	 ...
	 *
	 * $porCromo cambia la FORMA, no el criterio: una fila por cromo con
	 * `copias`, en vez de una por copia. El selector de `duelos.php` apila las
	 * copias («×N», y apuesta una cualquiera) y antes las agrupaba en PHP
	 * después de traerlas todas; con una colección grande eso son miles de
	 * filas con cinco JOIN cada una para pintar unos cientos.
	 *
	 * ⚠️ La VALIDACIÓN (`validarLoteApuesta()`) sigue llamando SIN agrupar, a
	 *    propósito. Agrupada solo aparecería una copia de cada cromo, y el día
	 *    que algo mande otra copia igual de legítima se rechazaría sin motivo.
	 *    La pantalla puede permitirse enseñar una; la comprobación no puede
	 *    permitirse aceptar solo una. `id_coleccion` agrupado es `MIN(...)` del
	 *    MISMO filtro, así que lo que ofrece la pantalla siempre pasa la
	 *    comprobación.
	 */
	public function listarCopiasApostables($id_usuario, $idRareza = null, $porCromo = false) {
		$sql = "
			SELECT
				" . ($porCromo ? "MIN(col.id_coleccion) AS id_coleccion, COUNT(*) AS copias," : "col.id_coleccion,") . "
				c.id_cromo, c.nombre, c.posicion, c.imagen,
				c.ataque, c.defensa, c.tecnica,
				eq.nombre AS equipo, c.universo,
				r.id_rareza, r.nombre AS rareza,
		...
		if ($porCromo) {
			// Se enumeran todas las columnas y no solo `c.id_cromo`: MariaDB
			// 10.4 no deduce que las demás dependen de la clave primaria, y con
			// ONLY_FULL_GROUP_BY activo la versión corta aborta.
			$sql .= "
			GROUP BY c.id_cromo, c.nombre, c.posicion, c.imagen,
				c.ataque, c.defensa, c.tecnica,
				eq.nombre, c.universo, r.id_rareza, r.nombre,
	...
	// ==========================================================
	// FICHAS PÚBLICAS DE CARTA (carta.php)
	//
	// Las 470 cartas del juego vivían todas dentro de una sola URL de 657 KB.
	// Eran 460 páginas potenciales convertidas en cero: nadie que busque el
	// nombre de un jugador de la liga encontraba nada.
	//
	// QUÉ ES PÚBLICO, y por qué exactamente esto:
	//   · Solo expansiones ACTIVAS. Lo retirado no se puede conseguir; darle
	//     página es indexar un catálogo muerto.
	//   · Nunca las exclusivas de cadena (`solo_cadena`). El álbum ya las
	//     esconde a propósito hasta que caen —«hay cartas secretas que solo
	//     aparecen cuando las consigues»—, y una página pública las destriparía.
	//   · Ninguna consulta de aquí mira la sesión. Estas páginas son iguales
	//     para todo el mundo, que es lo que permite cachearlas.
	// ==========================================================

	/** El filtro de "carta pública", en un sitio, para que las cuatro consultas de abajo no se contradigan. */
	private const SQL_CARTA_PUBLICA = "e.activo = 1 AND c.solo_cadena = 0";

	/**
	 * Todas las fichas públicas agrupadas por JUGADOR, no por carta.
	 *
	 * `Tom Skipper` existe en Legendario y en Épico: son la misma persona con
	 * dos cartas. Dos páginas casi idénticas serían contenido duplicado, así
	 * que van juntas en una y las versiones se enseñan dentro.
	 *
	 * Se agrupa por nombre + equipo porque hay cuatro nombres que juegan en dos
	 * equipos distintos (Fei Rune, Maddie Moonlight, Steve Grim y Zanark
	 * Avalonic); ahí sí son fichas diferentes y necesitan URL diferente.
	 *
	 * Devuelve una fila por página, con las versiones dentro de `versiones`.
	 */
	public function fichasPublicas(): array
	{
		$sql = "
			SELECT
				c.id_cromo, c.nombre, c.posicion, c.imagen, c.descripcion,
				c.ataque, c.defensa, c.tecnica, c.universo, c.cupo_numerado,
				c.id_equipo, eq.nombre AS equipo,
				c.id_rareza, r.nombre AS rareza, r.probabilidad,
				af.nombre AS afinidad, af.imagen AS afinidad_imagen,
				e.nombre AS expansion,
				(SELECT rg.nombre FROM cromo_rasgos cr INNER JOIN rasgos rg ON rg.id_rasgo = cr.id_rasgo
			...
			INNER JOIN rarezas     r ON r.id_rareza    = c.id_rareza
			INNER JOIN afinidad   af ON af.id          = c.id_afinidad
			WHERE " . self::SQL_CARTA_PUBLICA . "
			ORDER BY c.nombre ASC, c.id_rareza DESC
		";

		$fichas = [];
		foreach ($this->pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $fila) {
			$clave = $fila['nombre'] . '|' . $fila['id_equipo'];
			if (!isset($fichas[$clave])) {
				$fichas[$clave] = $fila;              // la de más rareza manda: el ORDER BY la pone primera
				$fichas[$clave]['versiones'] = [];
			}
			$fichas[$clave]['versiones'][] = $fila;
		}

		/* Un nombre que juega en dos equipos necesita el equipo en la URL para
		   distinguirse; los demás no, que alarga la URL sin aportar nada. Se
		   marca aquí, con el conjunto completo delante, porque desde una sola
		   fila es imposible saberlo. */
		$porNombre = [];
		foreach ($fichas as $f) { $porNombre[$f['nombre']] = ($porNombre[$f['nombre']] ?? 0) + 1; }
		foreach ($fichas as $k => $f) { $fichas[$k]['ambiguo'] = $porNombre[$f['nombre']] > 1; }

		return array_values($fichas);
	}

	/**
	 * La media de cada estadística por posición, sobre el catálogo público.
	 *
	 * Es lo que convierte «Defensa 96» —un número sin contexto— en «18 puntos
	 * por encima de la media de los porteros». Sin esto, las 460 fichas dirían
	 * lo mismo con cifras distintas, que es la definición de contenido delgado.
	 *
	 * Se excluyen las cartas de 0/0/0 (escudos, presidentes y entrenadores no
	 * juegan) para no hundir la media de las posiciones que sí compiten.
	 */
	public function mediasPorPosicion(): array
	{
		$sql = "
			SELECT c.posicion,
			       ROUND(AVG(c.ataque))  AS ataque,
			       ROUND(AVG(c.defensa)) AS defensa,
			       ROUND(AVG(c.tecnica)) AS tecnica,
			       COUNT(*)              AS cartas
			FROM cromos c
			INNER JOIN expansiones e ON e.id_expansion = c.id_expansion
			WHERE " . self::SQL_CARTA_PUBLICA . "
			  AND (c.ataque > 0 OR c.defensa > 0 OR c.tecnica > 0)
			GROUP BY c.posicion
		";
		$medias = [];
		foreach ($this->pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $f) {
			$medias[$f['posicion']] = $f;
		}
		return $medias;
	}
	...
	/**
	 * Compañeros de equipo, para enlazar entre fichas.
	 *
	 * No es adorno: son las 460 páginas enlazándose entre sí. Sin esto cada
	 * ficha sería un callejón sin salida al que solo se llega desde el catálogo,
	 * y un rastreador que entre por una no encuentra ninguna otra.
	 */
	public function companerosDeEquipo(int $idEquipo, string $excluirNombre, int $limite = 12): array
	{
		$stmt = $this->pdo->prepare("
			SELECT c.nombre, c.posicion, c.imagen, c.id_equipo, eq.nombre AS equipo,
			       r.nombre AS rareza, c.id_rareza
			FROM cromos c
			INNER JOIN expansiones e ON e.id_expansion = c.id_expansion
			INNER JOIN equipos    eq ON eq.id_equipo   = c.id_equipo
			INNER JOIN rarezas     r ON r.id_rareza    = c.id_rareza
			WHERE " . self::SQL_CARTA_PUBLICA . "
			  AND c.id_equipo = :eq AND c.nombre <> :n
			GROUP BY c.nombre
			ORDER BY c.id_rareza DESC, c.nombre ASC
			LIMIT " . max(1, $limite) . "
		");
		$stmt->execute([':eq' => $idEquipo, ':n' => $excluirNombre]);
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	}

	/**
	 * Los nombres que juegan en MÁS DE UN equipo dentro del catálogo público.
	 *
	 * Hoy son cuatro: Fei Rune, Maddie Moonlight, Steve Grim y Zanark Avalonic.
	 * Solo esos llevan el equipo en su URL (`/carta/fei-rune-gar`); el resto van
	 * con el nombre a secas, porque alargar 456 URL para desambiguar cuatro es
	 * empeorar 456.
	 *
	 * Quien enlaza a una ficha necesita saberlo ANTES de construir el enlace, o
	 * apuntaría a `/carta/tom-skipper-instituto-zeus`, que no existe. Se
	 * devuelve como mapa para preguntar con `isset()`.
	 */
	public function nombresAmbiguos(): array
	{
		$sql = "
			SELECT c.nombre
			FROM cromos c
			...
			WHERE " . self::SQL_CARTA_PUBLICA . "
			GROUP BY c.nombre
			HAVING COUNT(DISTINCT c.id_equipo) > 1
		";
		return array_fill_keys($this->pdo->query($sql)->fetchAll(PDO::FETCH_COLUMN), true);
	}

	/** Cuántas cartas públicas tiene un equipo (para el enlace «ver el equipo entero»). */
	public function contarCartasEquipo(int $idEquipo): int
	{
		$stmt = $this->pdo->prepare("
			SELECT COUNT(DISTINCT c.nombre)
			FROM cromos c
			INNER JOIN expansiones e ON e.id_expansion = c.id_expansion
			WHERE " . self::SQL_CARTA_PUBLICA . " AND c.id_equipo = :eq
		");
