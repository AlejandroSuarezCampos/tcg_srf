...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !csrfValido($_POST['csrf'] ?? null)) {
    ...
    if ($accion === 'crear') {
        ...
        if ($res['ok']) {
            header('Location: duelo?id=' . $res['id_duelo']);
            exit;
        }
        $error = $res['error'];
...
// El apilado lo hace ya la consulta (tercer argumento), que devuelve una fila
// por cromo con su número de `copias`.
$apostablesPorRareza = [];
$porCromoApostable   = $db->listarCopiasApostables($id_usuario, null, true);
foreach ($porCromoApostable as $f) {
    $f['cantidad'] = (int) $f['copias'];
    $apostablesPorRareza[(int) $f['id_rareza']][] = $f;
}
...
<main id="contenido" class="seccion wrap">
  ...
  <?php if (!$titular): ?>
    <div class="vacio">
      ...
      <a class="btn btn-primary" href="mazos">Ir a mazos</a>
    </div>
                ...
                <tr>
                  ...
                  <td style="text-align:right;">
                    <?php if ($d['estado'] !== 'cancelado'): ?>
                      <a class="btn btn-plano btn-sm" href="duelo?id=<?= $d['id_duelo'] ?>">Ver</a>
                    <?php endif; ?>
                  </td>
                </tr>
              <?php endforeach; ?>
