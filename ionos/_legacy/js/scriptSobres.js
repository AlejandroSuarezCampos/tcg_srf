...
// Si alguna es rareza SRF, se añade una pequeña celebración extra (glow + flash de pantalla).
function pintarCartasReveladas(cartas) {
  const grid = document.getElementById('revealGrid');
  grid.innerHTML = '';

  const hayGolpeSrf = cartas.some((c) => c.clase === 'r-srf');
  const modalReveal = document.querySelector('#modalRevelacion .modal-box-reveal');
  if (modalReveal) modalReveal.classList.toggle('srf-hit', hayGolpeSrf);

  cartas.forEach((carta, i) => {
    const item = document.createElement('div');
    item.className = 'reveal-card' + (carta.clase === 'r-srf' ? ' is-srf' : '');
