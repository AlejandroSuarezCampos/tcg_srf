...
$totalColeccion = $db->contarCromosTotales($id_usuario);
$totalObtenidas = $db->contarColeccionUsuario($id_usuario);
$porcentaje     = $totalColeccion > 0 ? round($totalObtenidas / $totalColeccion * 100) : 0;

// La colección nace en rejilla: es la vitrina, y aquí se viene a mirar el
// arte. La lista queda a un clic para cuando toque comparar estadísticas.
$vista = vista_actual('rejilla');
  ...
  <div class="con-filtros">
    ...
    <details class="filtros" data-plegable-movil>
      ...
      <form method="GET" class="stack stack-5 filtros-cuerpo">
        <?php // La vista viaja en la URL: se reenvía para que filtrar no la pierda. ?>
        ...
        <div class="stack stack-2">
          ...
          <?php if ($hayFiltros): ?>
          <a class="btn btn-plano btn-bloque" href="coleccion.php?vista=<?= htmlspecialchars($vista) ?>">Quitar filtros</a>
          <?php endif; ?>
        </div>
      </form>
    </details>
    ...
    <div>
      <div class="fila fila-entre" style="margin-bottom:var(--space-5);">
        ...
        <?php if (!empty($grupos)) { render_vista_conmutador($vista); } ?>
      </div>
      ...
      <?php if (empty($grupos)): ?>
        <div class="vacio">
          ...
          <?php if ($hayFiltros): ?>
            ...
            <a class="btn btn-ghost" href="coleccion.php?vista=<?= htmlspecialchars($vista) ?>">Quitar filtros</a>
          <?php else: ?>
      ...
      <?php else: ?>
        <<?= $vista === 'lista' ? 'ul class="carta-lista"' : 'div class="carta-grid"' ?>>
          <?php foreach ($grupos as $g): ?>
            ...
            $accionesFicha = [
                'acciones' => $esJugador ? json_encode([
                    ['texto' => 'Añadir al mazo', 'href' => 'mazos.php'],
            ...
            if ($vista === 'lista') {
                render_carta_fila($c, [
                    ...
                    'meta'    => htmlspecialchars($c['equipo'])
                        . ($cantidad > 1 ? ' · ×' . (int) $cantidad . ' copias' : ''),
                    'derecha' => $accion,
                    'datos'   => $accionesFicha,
                ]);
            } else {
                /* Modo arte: el arte manda y toda la ficha vive en el modal.
                   Las estadísticas ya no se pintan encima de la carta — el
                   modal las enseña con su color y su barra. */
                render_carta($c, [
                    'modo'     => 'arte',
                    'ficha'    => true,
                    'acciones' => $accion,
...
<?= assetScript($base ?? '', 'assets/js/coleccion.js') ?>
