...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !csrfValido($_POST['csrf'] ?? null)) {
    ...
    header('Location: cadena?id=' . $id_cadena);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'jugar') {
        ...
        if ($res['ok']) {
            ...
            header('Location: duelo?id=' . $res['id_duelo'] . (!empty($res['reanudado']) ? '&reanudado=1' : ''));
            exit;
        }
        $_SESSION['cadena_error'] = $res['error']
            . (!empty($res['cartas_excedidas'])
                ...
                : '');
        header('Location: cadena?id=' . $id_cadena . '&nodo=' . (int) ($_POST['id_nodo'] ?? 0));
        exit;
    }
...
<header class="cabecera">
  ...
  <div class="wrap cabecera-contenido">
    <p class="t-caption t-dim"><a href="cadenas">Cadenas</a></p>
    ...
    <div class="mapa" style="width:<?= $ancho ?>px; height:<?= $alto ?>px;">
      ...
      <?php foreach ($nodos as $id => $n): ?>
        ...
        <div class="<?= implode(' ', $clases) ?>"
          ...
          <?php if ($n['disponible']): ?>
            <a class="nodo-boton" href="cadena?id=<?= $id_cadena ?>&nodo=<?= $id ?>#seleccion">
          <?php else: ?>
...
(function () {
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      /* `form.action` y no 'cadena.php?id=N' escrito a mano. El formulario no
         lleva `action`, así que eso ES la URL de esta página, que es la misma a
         la que envía el formulario cuando no hay JavaScript.

         Con 'cadena.php' el `.htaccess` respondía 301 hacia /cadena —quita el
         .php de todas las URL— y un 301 convierte el POST en GET y tira el
         cuerpo: llegaba una petición sin `accion` ni `id_nodo`, la página se
         pintaba entera como HTML normal, y `res.json()` reventaba con
         «Unexpected token '<', "<!DOCTYPE "... is not valid JSON». El cofre no
         se abría y el error solo se veía en la consola. */
