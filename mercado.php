...
if ($esPostMutante && $_POST['accion'] === 'publicar') {
    ...
    if ($id_coleccion <= 0 || $precio <= 0) {
        ...
        header('Location: mercado?ok=publicado');
        exit;
    }
}
...
if ($esPostMutante && $_POST['accion'] === 'retirar') {
    ...
    header('Location: mercado?ok=retirado');
    exit;
}
...
if ($esPostMutante && $_POST['accion'] === 'comprar') {
    ...
    if ($resultado['ok']) {
        header('Location: mercado?ok=comprado');
        exit;
    } else {
        $error = $resultado['error'];
    }
...
// El agrupado lo hace ya `listarColeccionVendible()`, que devuelve una fila por
// cromo con su número de `copias` libres.
$porCromoVendible = $db->listarColeccionVendible($id_usuario);
...
<?php cabecera([
  ...
  /* Los intercambios (migración `053`) son la otra mitad de mover cartas, y
     no tienen sitio en la navegación —son cinco destinos fijos—, así que se
     entra desde aquí: quien viene a mover una carta ya está en esta pantalla. */
  'accion' => '<span class="fila">'
            . '<a class="btn btn-ghost" href="intercambios">'
            . '<i class="ph ph-swap" aria-hidden="true"></i> Intercambios</a>'
...
<main id="contenido" class="seccion wrap">
  ...
  <form method="GET" class="barra-filtros">
    ...
    <div class="barra-filtros-acciones">
      ...
      <?php if ($hayFiltros): ?>
      <a class="btn btn-plano" href="mercado?vista=<?= htmlspecialchars($vista) ?>">Quitar</a>
      <?php endif; ?>
    </div>
  </form>
  ...
  <?php if (empty($anuncios)): ?>
    <div class="vacio">
      ...
      <?php if ($hayFiltros): ?>
        ...
        <a class="btn btn-ghost" href="mercado">Quitar filtros</a>
      <?php else: ?>
  ...
  <?php else: ?>
    ...
    <<?= $vista === 'lista' ? 'ul class="carta-lista"' : 'div class="carta-grid"' ?>>
      <?php foreach ($anuncios as $a): ?>
        ...
        $formulario = '<form method="POST" action="mercado" class="js-mercado" data-confirmar="'
        ...
        $vendedor = '<span class="carta-vendedor"><span class="avatar avatar--sm">' . htmlspecialchars($iniciales)
            . '</span>Vende ' . ($esTuyo
                ...
                : '<a href="usuario?u=' . (int) $a['id_vendedor'] . '">'
...
<div class="modal" id="modalVender" role="dialog" aria-modal="true"
  ...
  <div class="modal-caja">
    ...
    <?php else: ?>
      <form method="POST" action="mercado" class="stack stack-5" id="formVender">
        <?= csrfCampo() ?>
        ...
        <div class="selector-cartas" id="v-lista" role="radiogroup" aria-label="Cartas disponibles para vender">
          <?php foreach ($porCromoVendible as $c): ?>
            ...
            <label class="selector-item"
              ...
              <?php render_carta($c, ['tamano' => 'sm', 'cantidad' => (int) $c['copias']]); ?>
            </label>
          <?php endforeach; ?>
