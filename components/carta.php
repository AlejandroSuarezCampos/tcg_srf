...
/**
 * Codifica una ruta de imagen para poder meterla en un atributo.
 *
 * ⚠️ EN `srcset` UN ESPACIO SIN CODIFICAR NO ES UN DETALLE: ES EL SEPARADOR.
 *    El formato de cada candidato es «URL espacio descriptor», así que
 *    `.../Cromos/Apuesta Segura/torch-256w.webp 256w` se lee como la URL
 *    `.../Cromos/Apuesta` con el descriptor `Segura/torch-256w.webp`, que no es
 *    válido, y el navegador tira el candidato entero. Medido: con las rutas sin
 *    codificar el navegador seguía bajando el original de 187 KB en un móvil,
 *    como si no hubiera `srcset`.
 *
 *    En `src` el espacio se perdona —el navegador lo codifica solo—, pero se
 *    codifica también aquí por coherencia y porque algunos rastreadores no lo
 *    perdonan (el catálogo tiene además carpetas con acentos, como
 *    `InazumaWorldCup/España`).
 *
 * Se codifica SEGMENTO A SEGMENTO: las barras y los `.`/`..` del principio
 * tienen que sobrevivir, o la ruta deja de apuntar a ningún sitio.
 */
function carta_url(string $ruta): string
{
    return implode('/', array_map(
        function (string $t): string {
            if ($t === '' || $t === '.' || $t === '..') { return $t; }
            // Un segmento que ya venga codificado no se codifica dos veces.
            return rawurlencode(rawurldecode($t));
        },
        explode('/', $ruta)
    ));
}
...
/**
 * El `srcset` de una ilustración, si tiene variantes generadas.
 *
 * Devuelve la cadena lista para el atributo, o '' cuando no hay nada que
 * ofrecer — que es el caso de la inmensa mayoría: 507 de las 538 ilustraciones
 * ya miden 256 px y pesan unos 8 KB, y la carta se pinta entre 150 y 280 px, así
 * que ahí el tamaño bueno ES el original y un `srcset` no ahorraría un byte.
 *
 * Las variantes las escribe `db/herramientas/generar_variantes.php`, y solo para
 * el arte que de verdad está sobredimensionado: 31 cartas de 760 a 1130 px que
 * se llevan el 46 % de los megas del catálogo, todas Legendario, SRF o Numerada
 * —justo las que salen en la portada y las que la gente abre en su ficha—.
 * Una de ellas pasa de 187 KB a 37 KB.
 *
 * ⚠️ SE COMPRUEBA EN DISCO, no se da por hecho. Si alguien sube un arte nuevo y
 *    no vuelve a pasar el generador, esta función simplemente no ofrece
 *    variantes y la carta se sirve como siempre. Nunca apunta a un fichero que
 *    no existe.
 *
 * El resultado se memoriza por petición: una rejilla repite la misma carta
 * varias veces y sin caché se harían cientos de `is_file()` de más.
 */
function carta_srcset(string $src): string
{
    static $cache = [];
    if (isset($cache[$src])) { return $cache[$src]; }

    // El `src` puede venir con './' (raíz) o '../' (las fichas, que cuelgan de
    // /carta/). Lo que hace falta es la ruta en disco, que sale de este fichero.
    $relativa = ltrim(preg_replace('#^(\.\./|\./)+#', '', $src), '/');
    $disco    = __DIR__ . '/../' . $relativa;

    $candidatos = [];
    foreach ([256, 512] as $w) {
        $variante = preg_replace('/\.webp$/i', "-{$w}w.webp", $disco);
        if ($variante !== null && is_file($variante)) {
            $candidatos[] = carta_url(preg_replace('/\.webp$/i', "-{$w}w.webp", $src)) . ' ' . $w . 'w';
        }
    }

    // Sin variantes no hay nada que decidir: se sirve el original y punto.
    if (!$candidatos) { return $cache[$src] = ''; }

    // El original entra como candidato mayor, con su ancho real: es el que debe
    // ganar en una pantalla grande de doble densidad.
    $medidas = @getimagesize($disco);
    if ($medidas && $medidas[0] > 0) { $candidatos[] = carta_url($src) . ' ' . (int) $medidas[0] . 'w'; }

    return $cache[$src] = implode(', ', $candidatos);
}
...
function render_carta_fila(array $c, array $opts = []): void
{
    ...
    <li class="<?= implode(' ', $clasesFila) ?>" data-rareza="<?= $idRareza ?>"<?= $attrs ?>>
      ...
      <span class="carta-fila-miniatura<?= $miniMarco ? ' carta-fila-miniatura--marco' : '' ?>"
        ...
        <?php if ($imagen !== ''): ?>
          <img src="<?= htmlspecialchars(carta_url($imagen)) ?>"
...
function render_carta(array $c, array $opts = []): void
{
    ...
    $datos        = $opts['datos']        ?? [];
    $lazy         = $opts['lazy']         ?? true;
    /* LA IMAGEN QUE DECIDE EL LCP.
       Lighthouse medía 5,4 s de LCP en el catálogo y señalaba como culpable
       justo la primera carta: llevaba `loading="lazy"`, así que el navegador
       aplazaba precisamente la imagen con la que se cronometra la página.
       `fetchpriority="high"` le dice que esa va primero. Solo vale para UNA
       o dos por página: si se marcan todas, no se prioriza ninguna. */
    $prioridad    = $opts['prioridad']    ?? false;
    /* A qué ancho se pinta la carta, para que el navegador elija bien del
       `srcset`. El valor por defecto cubre los dos sitios donde hay arte
       grande: la rejilla del catálogo (dos columnas en móvil) y la ficha de
       carta, que la fija en 260-280 px. Una pantalla concreta puede pasar el
       suyo si se desvía mucho de eso. */
    $sizes        = $opts['sizes']        ?? '(max-width: 720px) 45vw, 280px';
    if ($prioridad) { $lazy = false; }
              ...
              <div class="carta-foto-hueco">
                <?php if ($imagen !== ''): ?>
                  <img class="carta-arte"
                       src="<?= htmlspecialchars(carta_url($imagen)) ?>"
                       <?php $ss = carta_srcset($imagen); if ($ss !== ''): ?>srcset="<?= htmlspecialchars($ss) ?>" sizes="<?= htmlspecialchars($sizes) ?>"<?php endif; ?>
                       alt="Ilustración de <?= htmlspecialchars($nombre) ?>"
                       <?= $lazy ? 'loading="lazy" decoding="async"' : ($prioridad ? 'fetchpriority="high" decoding="async"' : 'decoding="async"') ?>>
                <?php else: ?>
