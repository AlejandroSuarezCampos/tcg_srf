...
/* El álbum se ve sin sesión (es el escaparate del juego). Sin usuario no hay
   secretas que desvelar, así que se pasa 0 y sale el catálogo público. */
$id_usuario = (int) ($_SESSION['id_usuario'] ?? 0);

$coleccion = $db->listarColeccionCompleta($id_usuario);
        ...
        <section class="expansion-grupo">
          ...
          <div class="carta-grid">
            <?php foreach ($expansion['cromos'] as $cromo): ?>
              ...
              /* Modo arte: el álbum es una vitrina, así que el arte manda y
                 la ficha se abre al pulsar. Las estadísticas ya no se pintan
                 sobre la carta — las enseña el modal, con su color.
                 Los `data-*` de aquí son los que lee el filtro de album.js;
                 se pasan explícitamente porque `afinidad` tiene que llegar en
                 crudo (incluido "No-afi") para que el desplegable case. */
              render_carta($cromo, [
                  'modo'    => 'arte',
                  'ficha'   => $tienePoseida,
