...
> ## ⚠️ ESTE DOCUMENTO TIENE UNA PARTE OBSOLETA (26 agosto 2026)
>
> Habla de subir el contenido de una carpeta **`ionos/`**. Esa carpeta **ya no
> existe**: se archivó en `_archivo/ionos/` porque estaba unos 60 ficheros por
> detrás de la raíz y a su `db/consultas.php` le faltaban métodos enteros. La
> adaptación que justificaba tenerla aparte —`charset=utf8mb4` en el DSN— ya
> está en la raíz, así que no queda ninguna diferencia de código que mantener.
>
> **Ahora se despliega desde la raíz del proyecto**, excluyendo
> `db/conexion.local.php`, `_archivo/`, `tcgfrontier.es-audit/`,
> `node_modules/`, `package*.json` y las carpetas de documentación
> (`branding/`, `design-system/`, `referencias-ux/`).
>
> Donde este documento diga «el contenido de `ionos/`», léase «el contenido de
> la raíz del proyecto, menos esa lista».
>
> Para la subida concreta del trabajo de SEO y de las fichas de carta, con su
> orden, sus comprobaciones y qué hacer si algo falla:
> **[`DESPLIEGUE-SEO.md`](DESPLIEGUE-SEO.md)**.
