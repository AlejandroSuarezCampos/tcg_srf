...
(function () {
  ...
  function decidir(idEvento, clave) {
    ...
    fetch('assets/ajax/duelo_minijuego', {
      method: 'POST',
  ...
  function elegirHueco(zona) {
    ...
    cuerpo.append('csrf', SRF.csrfToken());
    fetch('assets/ajax/duelo_penalti', { method: 'POST', body: cuerpo })
      .then(function (r) { return r.json(); })
  ...
  function sondear() {
    if (terminado) return;
    fetch('assets/ajax/duelo_narracion?id_duelo=' + encodeURIComponent(simulacion.dataset.idDuelo), {
      headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
      .then(function (r) { return r.json(); })
      .then(function (d) {
  ...
  /* PRIMERO LA PRESENTACIÓN, DESPUÉS EL PARTIDO.
     La intro de alineaciones (assets/js/presentacion.js) va justo aquí: el
     jugador acaba de elegir su aumento y todavía no ha visto el marcador. Se
     ESPERA a que termine —o a que la salte— antes de abrir el partido, porque
     abrir los dos a la vez dejaría el reloj del encuentro corriendo por debajo
     de una animación que tapa la pantalla, y se perderían minutos de partido
     sin que nadie los viera.

     `presentacionPartido()` está siempre definida, haya intro o no: cuando no
     la hay devuelve una promesa ya resuelta y esto sigue de largo. Así este
     punto no tiene que saber si el duelo trae presentación. */
  var intro = (window.SRF && SRF.presentacionPartido)
    ? SRF.presentacionPartido()
    : Promise.resolve();
